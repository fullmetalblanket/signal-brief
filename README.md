...
SignalBrief bundles its Marp slide and YouTube transcript workflows locally in [`skills/`](skills/). They are project instructions, not external installs. On first use, the workflow checks the tools available in the active AI environment and uses the simplest available path.
...
2. Clone SignalBrief directly into that personal research home folder so the folder itself becomes the research workspace, with no nested `research/` directory.
